    ...
    <main>
      <header className="topbar">
        <a className="brand-link" href="#top" aria-label="Runes home">
          <Image src="/generated/runes/mark-inverse.svg" alt="" width={28} height={28} unoptimized />
          <strong>RUNES</strong>
        </a>
        <span className="topbar-note">PERSONAL IDENTITY INFRASTRUCTURE</span>
      ...
      <section className="workbench" id="top">
        ...
        <div className="specimen-panel">
          ...
          <div className={`specimen-stage variant-${variant.replace(".svg", "")}`}>
            <Image src={asset} alt={`${selected.name} ${variant.replace(".svg", "")} preview`} width={size} height={size} unoptimized />
          </div>
      ...
      <section className="surface-strip" aria-label={`${selected.name} platform previews`}>
        <article className="surface social-surface">
          <span>GITHUB / X</span>
          <div className="avatar-mask"><Image src={`/generated/${selected.slug}/social-avatar.svg`} alt="" width={1024} height={1024} unoptimized /></div>
        </article>
        <article className="surface dock-surface">
          <span>DOCK / IOS</span>
          <div className="app-mask"><Image src={`/generated/${selected.slug}/app-icon.svg`} alt="" width={1024} height={1024} unoptimized /></div>
        </article>
        <article className="surface menu-surface">
          <span>MACOS MENU BAR</span>
          <div className="fake-menubar"><i /><i /><i /><Image src={`/generated/${selected.slug}/menubar.svg`} alt="" width={16} height={16} unoptimized /></div>
        </article>
        <article className="surface browser-surface">
          <span>BROWSER</span>
          <div className="fake-browser"><Image src={`/generated/${selected.slug}/app-icon.svg`} alt="" width={18} height={18} unoptimized /><b>runes.dev</b></div>
        </article>
        <article className="surface deck-surface">
          <span>PRESENTATION</span>
          <Image src={`/generated/${selected.slug}/title-slide.svg`} alt="" width={1920} height={1080} unoptimized />
        </article>
      </section>
        ...
        <div className="rune-grid">
          {filtered.map((identity, index) => (
            <button className={`rune-card ${selected.slug === identity.slug ? "selected" : ""}`} key={identity.slug} type="button" onClick={() => setSelectedSlug(identity.slug)}>
              ...
              <Image src={`/generated/${identity.slug}/mark-duotone.svg`} alt={`${identity.name} Rune`} width={64} height={64} unoptimized />
      ...
      <footer><Image src="/generated/runes/mark.svg" alt="" width={28} height={28} unoptimized /><strong>RUNES</strong><span>AN OPEN-SOURCE PERSONAL IDENTITY SYSTEM BY SAMZONG</span><a href="https://github.com/samzong/Runes">SOURCE</a></footer>
    </main>
  );
}
